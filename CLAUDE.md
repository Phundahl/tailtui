...
- **ANSI 16-color only.** Styling uses exclusively `lipgloss.Color("0")`–`"15"`. No hex values anywhere. This makes the UI inherit and follow the user's terminal theme. All color choices live in `internal/styles` as a semantic palette (`Primary`, `Secondary`, `Subtle`, `Warn`, `Danger`, …) — add new meanings there rather than scattering raw codes.
...
internal/
  ...
  styles/styles.go          all lipgloss styling, ANSI palette, Box/Divider/Sparkline/Bar helpers
  tui/
...
- **Exit node state lives in the list items, not in `LocalStatus`.** `Peer.IsActiveExitNode` is the single source of truth; at most one peer has it set. `Peer.OffersExitNode` is a *capability* flag — `toggleExitNode` (`x`) is a no-op unless the highlighted peer offers exit-node service. When it does toggle, it rewrites every peer via `list.SetItem` — enabling the highlighted one and clearing the rest, or clearing all if it was already active. The dashboard's `Exit:` value is *derived* via `Model.activeExitNodeName()` (scans the list), so it stays in sync automatically; `LocalStatus.ExitNode` is left empty on purpose. The active node is marked in the list with a yellow `󰖟 EXIT` chip (`styles.ExitChip`) rendered outside the selection block so selection + exit-node states are both visible.
...
- **True floating modals (compositing).** `View` always renders the full base layout first, then `renderOverlay(base)` blits the modal on top so the dashboard/list/details stay visible behind it. lipgloss v1 has no overlay primitive, so `overlayCenter` does it by hand: for each background row covered by the modal it keeps the left columns (`ansi.Truncate`), splices in the modal line, then resumes the background past the modal (`ansi.TruncateLeft`) — both are ANSI-aware and carry SGR state across the cut, with explicit `\x1b[0m` resets isolating the three segments. This is why `internal/tui` depends directly on `github.com/charmbracelet/x/ansi`.
- **Modal opacity (no bleed-through).** Terminals have no transparency, so the modal must paint every cell. Three rules make it 100% opaque: (1) every content span uses a `styles.Modal*` style that bakes in `styles.ModalBg` (ANSI 8, dark gray) — there are NO foreground-only spans whose reset would expose the terminal default; (2) every line is padded to the exact content width via `modalLine`/`styles.ModalFill`, and the viewport height is sized to the content (`overlayHeight`) so there are no short padding lines; (3) the container sets explicit `Width`/`Height` **including** the padding (`w + 2*modalHPad`, etc.) — lipgloss counts padding inside `Width`, so omitting it shrinks the content area and wraps the full-width lines. Don't use `Subtle` (ANSI 8) for modal text; it equals `ModalBg` — dim with `Faint` instead.
- **Layout is centralized in `computeLayout`.** Both `Update` (which sizes the list via `SetSize`) and `View` (which draws) read the same geometry, so they never disagree. `WindowSizeMsg` is intercepted (not forwarded to the list) so the list gets the *pane* size, not the *terminal* size. Box border/padding math: `styles.Box(w,h)` renders to exact OUTER w×h (border included); content is sized `w-2`/`h-2`.
- **Color-by-state convention** (from the spec, applied consistently): green = active/online/direct, yellow = relayed/degraded, red = conflict/error, subtle/grey = offline. `●`/`○` mark online/offline.
